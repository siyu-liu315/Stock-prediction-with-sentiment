...
data <- read.csv(file = "S&P500Index.csv")
mydata <- transform(data, date = as.Date(as.character(data$caldt), "%Y%m%d"))

### plot time series

ggplot(data = mydata, aes(x = date, y = spindx)) +
  geom_line(color = "#FC4E07") +
  labs(title = "1990 - 2019 S&P500 Index")

try <- mydata %>% select(date, spindx)

create_ts <- function(x){
  ts(x, start = c(1995, 1), frequency = 12)
}

data_master <- lapply(try, create_ts)
View(data_master)

sp_500 <- ts(data_master, start=c(1995, 1), freq=12)

sp500_training <- ts(sp500, start=c(1990, 1), end=c(2018, 12), freq=12)
View(sp500_training)
sp500_training <- as.vector(sp500_training)

#### BOX test
Box.test(sp500_training, lag = 20, type = 'Ljung-Box')
adf.test(sp500_training)

plot_decomp(sp500_training)
